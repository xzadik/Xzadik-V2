...
class Racer:
    ...
    def field(self):
        field = "<:elixir:572793564568748042> **{}** :flag_black:  [{}]".format(self.current, self.user.display_name)
        return field
...
class Race:
    ...
    def getBSChars(self):
        ...
        return ((self.emoji('Shelly'), 'steady'),
                (self.emoji('Nita'), 'abberant'),
                (self.emoji('Colt'), 'steady'),
                ...
                (self.emoji('Rico'), 'slow'),
                (self.emoji('Darryl'), 'predator'),
                (self.emoji('Penny'), 'slow'),
                (self.emoji('Carl'), 'abberant'),
                (self.emoji('Piper'), 'slow'),
                (self.emoji('Pam'), 'slow'),
                (self.emoji('Frank'), 'abberant'),
                (self.emoji('Bibi'), 'steady'),
                (self.emoji('Mortis'), 'fast'),
                (self.emoji('Tara'), 'slow'),
                (self.emoji('Gene'), 'steady'),
                (self.emoji('Crow'), 'fast'),
                (self.emoji('Spike'), 'abberant'),
                (self.emoji('Leon'), 'fast'),
                (self.emoji('Boss'), 'special'))
    ...
    async def _prize_setrace(self, ctx, minimum: int, maximum: int):
        ...
        if minimum > maximum:
            return await self.context.send("https://simple.wikipedia.org/wiki/Maximum_and_minimum")
        guild = ctx.message.guild
        settings = self.check_config(guild)
        settings['Prize'] = (minimum, maximum)
        self.save_settings()
        await self.context.send("Prize range set to {}-{}".format(minimum, maximum))
    ...
    async def _cost_setrace(self, ctx, num: int):
        ...
        settings['Cost'] = num
        self.save_settings()
        await self.context.send("Cost set to {} credits.".format(num))
    ...
    async def _time_setrace(self, ctx, time: int):
        ...
        if time < 0:
            return await self.context.send("{}. You are a dumbass. I can't turn back"
                                      "time.".format(author.name))

        settings = self.check_config(author.guild)
        settings['Time'] = time
        self.save_settings()
        await self.context.send("Wait time set to {}s".format(time))
    ...
    async def _mode_setrace(self, ctx, mode: str):
        ...
        if mode not in modes:
            return await self.context.send("Invalid mode. Acceptable responses "
                                      "include: {}.".format(', '.join(modes)))
        settings['Mode'] = mode
        self.save_settings()
        await self.context.send("Mode now set to {}.".format(mode))
    ...
    async def _reset_race(self, ctx):
        ...
        await self.context.send("Parameters reset.")
    ...
    async def _enter_race(self, ctx):
        ...
        cooldown = self.check_cooldown(author.guild)
        settings = self.check_config(author.guild)
        cost = settings["Cost"]
        timer = 600
        ...
        if channel.name != "race":
            return await self.context.send("You cannot run this command in this channel. Please run this command at #race")

        if data['Race Active']:
            if author.id in data['Players']:
                return await self.context.send("You are already in the race!")
            elif not self.bank_check(settings, author):
                return await self.context.send("You do not meet the cost of entry. You need atleast {} credits.".format(cost))
            elif len(data['Players']) == 10:
                return await self.context.send("There are no more spots left in the race!")
            else:
                bank = self.bot.get_cog('Economy').bank
                bank.withdraw_credits(author, cost)
                data['Players'][author.id] = {}
                return await self.context.send("**{}** entered the race!".format(author.display_name))

        if time.time() - cooldown < timer:
            return await self.context.send("You need to wait {} before starting another race.".format(self.time_format(int(timer - (time.time() - cooldown)))))
        ...
        if raceRole is None:
            await self.bot.create_role(guild, name=role_name)
            raceRole = discord.utils.get(guild.roles, name=role_name)
        ...
        await self.bot.edit_role(guild, raceRole, mentionable=True)
        await self.context.send(":triangular_flag_on_post: {} has started a race! Type ``{}race enter`` "
        ...
        await self.context.send(":checkered_flag: The race is now in progress :checkered_flag:")

        data['Race Start'] = True

        perm = discord.PermissionOverwrite(send_messages=False, read_messages=False)
        await self.bot.edit_channel_permissions(ctx.message.channel, guild.default_role, perm)

        race_msg = await self.context.send('\u200b'+'\n'+'\n'.join([player.field() for player in racers]))
        ...
        await self.context.send(content=mention, embed=embed)

        self.game_teardown(data)

        self.cooldown[guild.id] = time.time()
    ...
    async def _claim_race(self, ctx):
        ...
        if all(v is None for v in [data['First'], data['Second'], data['Third']]):
            return await self.context.send("There is nothing to collect.")
        ...
        if prize == prize_pool:
            return await self.context.send("Scram kid. You didn't win nothing yet.")

        try:
            bank = self.bot.get_cog('Economy').bank
        except AttributeError:
            return await self.context.send("Economy is not loaded.")
        ...
        try:  # Because people will play games for money without a fucking account smh
            ...
            await self.context.send("We wanted to give you a prize, but you didn't have a bank "
                               "account.\nTo teach you a lesson, your winnings are mine this "
                               "time. Now go register!")
        else:
            await self.context.send("After paying for king's tax, entrance fees, and arena fees, "
    ...
    def check_cooldown(self, guild):
        if guild.id in self.cooldown:
            return self.cooldown[guild.id]
        else:
            self.cooldown[guild.id] = 0
            return self.cooldown[guild.id]
    ...
    def check_config(self, guild):
        if guild.id in self.config['guilds']:
            ...
            self.config['guilds'][guild.id] = {'Prize': (1, 100), 'Cost': 500, 'Mode': 'standard', 'Time': 60}
            self.save_settings()
            return self.config['guilds'][guild.id]
